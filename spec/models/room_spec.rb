...
RSpec.describe Room, type: :model do
  ...
  describe "validations" do
    ...
    it "is invalid without a location" do
      ...
      expect(room.errors[:location]).to include("Cannot be blank")
    end

    it "is invalid without a capacity" do
      room.capacity = nil
      expect(room).to_not be_valid
      expect(room.errors[:capacity]).to include("Cannot be blank")
    end

    it "is invalid with a non-numeric capacity" do
      room.capacity = "abc"
      expect(room).to_not be_valid
      expect(room.errors[:capacity]).to include("Is not valid input")
    end
